...
interface StudentDashboardShellProps {
  ...
  pendingCount: number;
  onNavChange: (nav: StudentNavItem) => void;
  onLogout: () => void;
  children: ReactNode;
}

const navItems: { id: StudentNavItem; label: string; icon: string }[] = [
        ...
        <aside className="hidden w-64 shrink-0 self-stretch border-r border-slate-200/80 bg-white lg:flex lg:w-72 xl:w-80">
          <div className="sticky top-16 flex h-[calc(100vh-4rem)] w-full flex-col overflow-y-auto px-5 py-6 xl:px-6">
            <div className="mb-8 flex items-center gap-3">
              <div className="sccl-avatar h-12 w-12">{initials || 'ST'}</div>
            ...
            <nav className="space-y-1">
              {navItems.map((item) => {
                ...
                const showBadge = item.id === 'history' && pendingCount > 0;
            ...
            <button
              type="button"
              onClick={onLogout}
              className="sccl-btn-secondary mt-auto w-full"
            >
              Log out
            </button>
          </div>
